	...
	protected void smartPaste(IDocument document, DocumentCommand command) {
		...
		try {
			if (document instanceof IStructuredDocument) {
				DefaultIndentationStrategy.placeMatchingBlanksForStructuredDocument((IStructuredDocument) document,
						helpBuffer, document.getLineOfOffset(command.offset), command.offset);
		...
		try {
			for (int i = 0; i < lines; i++) {
				...
				tempsb.append(tempdocument.get(region.getOffset(), region.getLength()).trim());
			}
		} catch (BadLocationException e) {
		}
		JobSafeStructuredDocument newdocument = new JobSafeStructuredDocument(new TwigSourceParser());
		String start = "<?php";
