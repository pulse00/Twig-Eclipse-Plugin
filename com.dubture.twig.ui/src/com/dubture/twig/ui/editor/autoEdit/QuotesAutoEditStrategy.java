...
public class QuotesAutoEditStrategy extends MatchingCharAutoEditStrategy {
	...
	private void autoAddPairQuote(IStructuredDocument document,
		...
		try {
			...
			String endState = FormatterUtils.getPartitionType(document,
					endOffset, true);
			if (startState == TwigPartitionTypes.TWIG_QUOTED_STRING
					|| endState == TwigPartitionTypes.TWIG_QUOTED_STRING
					|| (startState == TwigPartitionTypes.TWIG_DEFAULT
							&& endState == TwigPartitionTypes.TWIG_DEFAULT && document
							.getChar(endOffset - 1) == '\'')) {
	...
	private boolean isQuoteAllowed(String state, char quote) {
		return (state == TwigPartitionTypes.TWIG_DEFAULT)
