...
public class ImagesOnFileSystemRegistry {
	...
	private static final String IMAGE_DIR = "twig-images"; //$NON-NLS-1$
	...
	private File getTempDir() {
		try {
			File imageDir = TwigUICorePlugin.getDefault().getStateLocation().append(IMAGE_DIR).toFile();
			if (imageDir.exists()) {
			...
			if (!imageDir.isDirectory()) {
				// SymfonyUiPlugin
				// .logErrorMessage("Failed to create image directory " +
				// imageDir.toString()); //$NON-NLS-1$
				return null;
			}
			return imageDir;
