	...
	public void format(IStructuredDocumentRegion sdRegion) {
		...
		for (int i = 0; i < textRegions.size(); i++) {
			...
			if (textRegion instanceof TwigScriptRegion) {
				...
				try {
					twigTokens = scriptRegion.getTwigTokens(0, textRegion
							.getLength());
								...
								if (j > 0
										&& (twigTokens[j - 1].getType().equals(
												...
												|| twigTokens[j - 1]
														.getType()
														.equals(
																TwigRegionTypes.PHP_CURLY_CLOSE) || twigTokens[j - 1]
												.getType()
												.equals(
														TwigRegionTypes.TWIG_COMMENT_CLOSE))) {
...
//				return;
			...
			} else*/ if (isTwigCommentRegion(firstTokenType)) {
	...
	private boolean shouldReformat(IStructuredDocument document,
		...
		return ((checkedLineBeginState == TwigPartitionTypes.TWIG_DEFAULT)
				|| (checkedLineBeginState == TwigPartitionTypes.TWIG_COMMENT_TEXT)) || (checkedLineBeginState == TwigPartitionTypes.TWIG_COMMENT_TEXT);
	}
