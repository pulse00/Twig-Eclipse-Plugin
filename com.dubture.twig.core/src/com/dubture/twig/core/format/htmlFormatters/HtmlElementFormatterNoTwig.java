	...
	*/
	protected void formatChildNodes(IDOMNode node, HTMLFormatContraints contraints) {
		...
		while (child != null) {
			...
			IStructuredFormatter formatter = HTMLFormatterNoTwigFactory.getInstance().createFormatter(child,
	...
	protected void insertBreakAfter(IDOMNode node, HTMLFormatContraints contraints) {
		...
		if (next == null) { // last spaces
			...
			if (contraints != null && contraints.getFormatWithSiblingIndent()) {
				...
				IStructuredFormatter formatter = HTMLFormatterNoTwigFactory.getInstance().createFormatter(text,
						getFormatPreferences());
	...
	*/
	...
	protected void formatNode(IDOMNode node, HTMLFormatContraints contraints) {
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			if (container != null && (container.getFirstRegion().getType().equals(TwigRegionContext.TWIG_OPEN)
					|| container.getFirstRegion().getType().equals(TwigRegionContext.TWIG_STMT_OPEN))) {
				TwigFormatter phpFormatter = new TwigFormatter(attribute.getStartOffset(), attribute.getEndOffset());
