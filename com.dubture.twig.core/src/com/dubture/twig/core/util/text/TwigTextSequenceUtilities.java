		...
		if (tRegion != null && TwigTextSequenceUtilities.isTwigRegion(tRegion.getType())) {
			...
			try {
				...
				if (documentOffset == startOffset) {
					...
					startTokenRegion = twigScriptRegion.getTwigToken(offset - startOffset - 1);
				}
				while (true) {
				...
				textSequence = textSequence.subTextSequence(PHPTextSequenceUtilities.readForwardSpaces(textSequence, 0),
						textSequence.length());
