...
public class TwigIndexingVisitor extends TwigASTVisitor {
	...
	public boolean visit(Variable s) throws Exception {
		if (blockTag != null) {
			Logger.debugMSG("Indexing block reference " + s.getValue() + " in " + module.getElementName() + " / "
					+ module.getScriptProject().getElementName());
			ReferenceInfo info = new ReferenceInfo(ITwigModelElement.BLOCK, s.sourceStart(),
					s.sourceEnd() - s.sourceStart(), s.getValue(), null, null);
	...
	public boolean visit(StringLiteral s) throws Exception {
		if (extendsTag != null) {
			...
			ReferenceInfo info = new ReferenceInfo(ITwigModelElement.EXTENDS, s.sourceStart(),
					s.sourceEnd() - s.sourceStart(), s.getValue(), null, null);
			requestor.addReference(info);
		}
		return false;
