...
public class TwigScriptRegion extends ForeignRegion implements ITwigScriptRegion {
	...
	public TwigScriptRegion(String newContext, int startOffset, IProject project, AbstractTwigLexer twigLexer,
			String lexerState) {
		...
		try {
			...
			ST_TWIG_CONTENT = twigLexer.getClass().getField(lexerState).getInt(twigLexer);
		} catch (Exception e) {
	...
	public StructuredDocumentEvent updateRegion(Object requester, IStructuredDocumentRegion flatnode, String changes,
		...
		try {
			...
			while (TwigRegionTypes.UNKNOWN_TOKEN.equals(tokenStart.getType()) && (tokenStart.getStart() > 0)) {
			...
			if (previousIndex >= 0) {
				ITextRegion previousRegion = tokensContainer.twigTokens.get(previousIndex);

				if (tokenStart.getType().equals(TwigRegionTypes.TWIG_COMMENT) && tokenStart.getLength() == 1
						&& previousRegion.getType().equals(TwigRegionTypes.TWIG_COMMENT_OPEN)) {
			...
			final LexerState endState = tokensContainer.getState(tokenEnd.getEnd() + 1);

			final TwigTokenContainer newContainer = new TwigTokenContainer();
			final TwigLexer twigLexer = getTwigLexer(
					new DocumentReader(flatnode, changes, requestStart, lengthToReplace, newTokenOffset), startState);
			...
			try {
				...
				while (yylex != null && newTokenOffset <= toOffset && (yylex != TwigRegionTypes.TWIG_COMMENT_CLOSE
			...
			// 1. replace the regions
			final ListIterator<ITextRegion> oldIterator = tokensContainer.removeTokensSubList(tokenStart, tokenEnd);
	...
	private void setTwigtokens(AbstractTwigLexer lexer) {
		...
		try {
			...
			int yylength = 0;
			while (yylex != null && (yylex != TwigRegionTypes.TWIG_COMMENT_CLOSE
					&& yylex != TwigRegionTypes.TWIG_CLOSETAG && yylex != TwigRegionTypes.TWIG_STMT_CLOSE)) {
